...
/*   Updated: 2023/02/19 17:02:05 by bamrouch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	fdf_init_block_info(t_fdf *fdf, int input_count,
		char *input_value[])
{
	int	block_size;
	int	default_color;
	...
	{
		block_size = log2(1024 + (fdf->map.x_count * fdf->map.y_count));
...
void	fdf_parser(t_fdf *fdf, int input_count, char *input_value[])
{
	...
	if (!(input_count >= 2 && input_count <= 4
			&& fdf_input_is_valid_file_name(input_value[1])))
